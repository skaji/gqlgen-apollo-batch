...
func TestBatchPOST(t *testing.T) {
	...
	t.Run("success3", func(t *testing.T) {
		resp := doRequest(h, "POST", "/graphql", `[{"query": "query($id:Int!){find(id:$id)}","variables":{"id":1}},{"query": "query($id:Int!){find(id:$id)}","variables":{"id":2}}]`)
		assert.Equal(t, http.StatusOK, resp.Code, resp.Body.String())
		assert.Equal(t, resp.Header().Get("Content-Type"), "application/json")
		assert.Equal(t, `[{"data":{"name":"test"}},{"data":{"name":"test"}}]`, resp.Body.String())
	})
	t.Run("success and fail -> 200 OK", func(t *testing.T) {
		resp := doRequest(h, "POST", "/graphql", `[{"query": "query($id:Int!){find(id:$id)}","variables":{"id":1}},{"query": "query($id:Int!){find(id:$id)}","variables":{"id":false}}]`)
		assert.Equal(t, http.StatusOK, resp.Code, resp.Body.String())
		assert.Equal(t, resp.Header().Get("Content-Type"), "application/json")
		assert.Equal(t, `[{"data":{"name":"test"}},{"errors":[{"message":"cannot use bool as Int","path":["variable","id"],"extensions":{"code":"GRAPHQL_VALIDATION_FAILED"}}],"data":null}]`, resp.Body.String())
	})
	...
	t.Run("must not be an empty array", func(t *testing.T) {
		...
		assert.Equal(t, http.StatusBadRequest, resp.Code, resp.Body.String())
		assert.Equal(t, resp.Header().Get("Content-Type"), "application/json")
		assert.Equal(t, `{"errors":[{"message":"json body must not be an empty array"}],"data":null}`, resp.Body.String())
	})
